...
const outputSchema = z
	.object({
		pageStatus: z
			.object({
				status: z.enum(['AVAILABLE_PRODUCT', 'NOT_AVAILABLE_PRODUCT', 'UNKNOWN_STATUS']).meta(
					{
						description: `
			...
			.meta({
				description: 'Information about the product availability status determined from the webpage content.',
			}),
	})
	.meta({
		description: 'The result object returned by the assistant.',
	});
